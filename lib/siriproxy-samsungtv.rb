...
class SiriProxy::Plugin::SamsungTv < SiriProxy::Plugin
  ...
  def turn_tv_off
	...
	Thread.new {
	      status = system("cd /www/automate/ && ./tv.pl key=POWEROFF")
  ...
  def turn_tv(key, startText="", returnText="")
	...
	Thread.new {
	      status = system("cd /www/automate/ && ./tv.pl key=#{key}")
  ...
  def turn_volume(direction, number)
	...
	Thread.new {
	      ...
	      while i < number
		      i += 1
		      status = system("cd /www/automate/ && ./tv.pl key=VOL#{direction.upcase}")
	      end
